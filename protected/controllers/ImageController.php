...
class ImageController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('display', 'index','view'),
				'users'=>array('*'),
			),
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create', 'process', 'update'),
				'users'=>array('@'),
			),
    ...
    public function actionProcess()
    {
        if (app()->request->isAjaxRequest) {
            if (is_uploaded_file($_FILES['file']['tmp_name']) && getimagesize($_FILES['file']['tmp_name']) != false){
                ...
                Yii::import('ext.dropzone.components.UploadImageComponent');

                $uploader = new UploadImageComponent($config);
    ...
    {
        $model = Image::model()->findByAttributes(array('long_id' => $img));

        if ($model == NULL) {
            header("Content-type: image/jpg");
            header("Content-Disposition: inline; filename=no-image.jpg");
            readfile(Yii::getPathOfAlias('webroot.images') . DIRECTORY_SEPARATOR . 'no_image.jpg');
        } else {
            $model->display();
        }
